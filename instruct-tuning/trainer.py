...
# Slighlty modified by Bram Vanroy to address this issue: https://github.com/lvwerra/trl/issues/467
...
class SFTTrainer(Trainer):
    ...
    def _prepare_non_packed_dataloader(
        ...
        def tokenize(batch):
            outputs = tokenizer(
                batch[dataset_text_field] if not use_formatting_func else formatting_func(batch),
                truncation=True,
                padding=True,
            ...
            if use_formatting_func and not self._dataset_sanity_checked:
                if not isinstance(formatting_func(batch), list):
                    raise TypeError(
