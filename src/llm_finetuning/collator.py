...
class DataCollatorForTurnBasedLM(DataCollatorForLanguageModeling):
    def __init__(
        self, user_token: str, assistant_token: str, *args, mlm: bool = False, ignore_index: int = -100, **kwargs
    ):
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        for sample_idx in range(len(examples)):
            ...
            for start_hum_idx in np.where(batch["labels"][sample_idx] == self.human_token_ids[0])[0]:
                if (
                    self.human_token_ids
                    == examples[sample_idx]["input_ids"][start_hum_idx : start_hum_idx + self.human_tokens_len]
                ):
            ...
            for start_ast_idx in np.where(batch["labels"][sample_idx] == self.assistant_token_ids[0])[0]:
                if (
                    ...
                    == examples[sample_idx]["input_ids"][start_ast_idx : start_ast_idx + self.assistant_tokens_len]
                ):
            ...
            if len(human_start_idxs) != len(assistant_start_idxs):
                raise ValueError(
                    f"Expected an equal amount of turns between assistant and human. Not the case in"
